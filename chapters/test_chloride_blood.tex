...
The chloride blood test measures the concentration of chloride ions (Cl\textsuperscript{-}) in the blood serum or plasma. Chloride is the major extracellular anion, accounting for approximately 70\% of the total anion content in extracellular fluid. Normal serum chloride ranges from 96--106 mEq/L (mmol/L) \cite{wallach2022interpretation}. Chloride plays essential roles in maintaining osmotic pressure, fluid balance, and acid-base homeostasis \cite{tierney2023current}. It is the main counter-ion to sodium, so chloride levels generally parallel sodium levels in most clinical situations. Chloride is also critical for gastric acid (HCl) secretion by gastric parietal cells and for the chloride-bicarbonate exchange in red blood cells (the ``chloride shift'' or Hamburger phenomenon), which is essential for CO\textsubscript{2} transport. Renal chloride reabsorption occurs mainly in the proximal tubule, thick ascending limb of Henle (via Na-K-2Cl cotransporter, the target of loop diuretics), and in the distal tubule (via Na-Cl cotransporter, the target of thiazide diuretics). Chloride is routinely measured as part of the basic metabolic panel (BMP) and comprehensive metabolic panel (CMP), and is essential for calculating the anion gap [Anion Gap = Na -- (Cl + HCO\textsubscript{3})].

\section*{Alternative Names}
\begin{itemize}
  \item Cl
  \item Serum Chloride
  \item Plasma Chloride
  \item Blood Chloride
  \item Cl- Level
  \item CSF Chloride (cerebrospinal fluid chloride measurement)
\end{itemize}
\section*{Principle}
...
\begin{itemize}
  ...
  \item Evaluation and monitoring of acid-base disorders. Chloride is essential for calculating the anion gap [Na -- (Cl + HCO\textsubscript{3})], which is required to differentiate types of metabolic acidosis \cite{kraut2007serum}
...
Chloride measurement is mainly a component of the electrolyte panel, making it both a primary screening test (as part of the BMP/CMP) and a secondary parameter, because isolated chloride abnormalities are rarely diagnostic alone and must be interpreted in the context of sodium, bicarbonate, anion gap, and acid-base parameters. Chloride is essential for the calculation of the anion gap, which is a cornerstone of metabolic acidosis evaluation.
...
\begin{itemize}
  \item No absolute contraindications for venipuncture. Important analytical interferences:
  \item (1) Bromide and iodide ions---both are sensed by chloride ISE with greater affinity than chloride itself, causing pseudohyperchloremia. This can result in a falsely low or even negative calculated anion gap. A negative anion gap should always trigger investigation for bromide toxicity.
  \item (2) Salicylate and thiocyanate are also highly sensed by some chloride electrodes and can cause falsely elevated chloride.
  \item (3) Lipemic samples---extreme hypertriglyceridemia can cause spurious chloride results with indirect ISE methods.
  \item (4) Drugs containing chloride salts (ammonium chloride, arginine hydrochloride, potassium chloride) can cause true hyperchloremia.
  \item (5) Hemolysis does not significantly affect chloride measurement.
  \item (6) Volume depletion/dehydration causes a proportional increase in chloride along with sodium.
  \item (7) Large-volume normal saline resuscitation (0.9\% saline) contains supraphysiologic chloride (154 mEq/L vs. normal plasma ~103 mEq/L) and can cause hyperchloremic metabolic acidosis.
  \item (8) Recent consumption of a high-salt meal has minimal effect on serum chloride in individuals with normal renal function.
\end{itemize}
\section*{Risks}
...
\begin{itemize}
  ...
  \item \textbf{Gastrointestinal losses (most common):} Persistent vomiting; nasogastric (NG) suction---loss of HCl-rich gastric fluid causes metabolic alkalosis with hypochloremia and hypokalemia \cite{gennari2008acid}. With NG suction, each liter of gastric fluid contains approximately 120--160 mEq of chloride. Severe diarrhea can also lower chloride.
  ...
  \item \textbf{Metabolic alkalosis with low urine chloride (<10--20 mEq/L):} Chloride-responsive metabolic alkalosis indicating volume depletion and need for chloride repletion \cite{galla2000metabolic}. Causes: vomiting, NG suction, post-hypercapnia alkalosis, diuretic use (remote). This responds to normal saline (NaCl) administration.
...
\begin{itemize}
  \item \textbf{Adults:} 96--106 mEq/L (mmol/L) \cite{fischbach2023manual}

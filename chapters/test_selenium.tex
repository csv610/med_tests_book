...
\begin{itemize}
  \item There are no absolute contraindications to standard venipuncture. Important pre-analytical and interpretive cautions:
  \item (1) Contamination of the specimen (contact with rubber stoppers, glass, or the skin surface) falsely elevates results; trace-element-free tubes are mandatory.
  \item (2) Hemolysis invalidates the sample because red cells concentrate selenium.
  \item (3) Serum selenium reflects only recent intake and may be normal despite subclinical tissue deficiency; whole-blood selenium is preferable for longer-term status.
  \item (4) The acute-phase response lowers serum selenium during infection, surgery, and critical illness, so low values must be interpreted with inflammatory markers in mind.
  \item (5) Selenium deficiency and toxicity have a narrow therapeutic window; supplementation decisions should be guided by both clinical and laboratory assessment.
  \item (6) Recent selenium supplements (e.g., 100--200 mcg daily) raise serum levels, so the history of supplementation is essential.
  \item (7) Renal failure and hemodialysis lower selenium levels.
  \item (8) Because selenium is excreted largely in urine, urine selenium is used specifically to assess excessive exposure.
\end{itemize}
\section*{Risks}

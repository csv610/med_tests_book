...
\begin{itemize}
  \item There are no absolute contraindications to the blood draw. The screen is not performed beyond the gestational-age window for each component, and women who have already undergone diagnostic testing do not need screening. Precautions: This is a screening test, not a diagnostic test, and a screen-positive result must be confirmed by chorionic villus sampling (CVS) or amniocentesis. Interpretation cautions:
  \item (1) Twin and higher-order multiple gestations require specialized risk algorithms, and marker concentrations differ from singletons.
  \item (2) Pregnancies conceived by in vitro fertilization or with oocyte donation, and pregnancies in women of different racial or ethnic groups, with insulin-dependent diabetes, or with vaginal bleeding require adjustments to the multiples-of-the-median (MoM) calculations.
  \item (3) Nuchal translucency measurement is operator-dependent and must be performed by certified sonographers using a standardized technique.
  \item (4) An elevated MSAFP may reflect gestational dating errors, multiple gestation, abdominal wall defects, or fetal demise in addition to open neural tube defects, and requires targeted ultrasound evaluation.
\end{itemize}
\section*{Risks}

...
The numerical intervals below are illustrative teaching examples, not universal decision limits. The authoritative interval is the one printed on the patient's report by the laboratory that performed the assay. Intervals depend on method, calibration, specimen type, age, sex, pregnancy, population, and local validation; some tests use clinical decision limits rather than a healthy-population reference interval.

A few critical points before you start reading:
\begin{itemize}
    \item Reference ranges vary by lab. The numbers in this appendix are typical examples, but the performing laboratory's interval takes precedence.

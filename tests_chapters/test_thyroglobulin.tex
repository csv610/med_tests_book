...
The primary contraindication is the invalidity of Tg as a tumor marker in the presence of Tg autoantibodies. TgAb renders immunometric Tg results unreliable (falsely low). Therefore, TgAb must be measured on every Tg sample. If TgAb is positive, Tg results should not be used for clinical decision-making; changes in TgAb titer become the main biochemical indicator. Pre-analytical factors: Tg levels are TSH-dependent; therefore, the TSH level at the time of measurement must be known to interpret the result (suppressed vs. stimulated Tg). Patients with incomplete surgery (lobectomy only) will have residual Tg from the remaining lobe, making Tg less useful for cancer surveillance. Thyroglobulin half-life is 1--3 days after thyroidectomy; Tg should not be measured for 6--8 weeks after surgery or radioactive iodine therapy to allow clearance. Heterophile antibodies and human anti-mouse antibodies (HAMA) can cause falsely elevated Tg results in sandwich immunoassays. Recent iodine-containing contrast agents transiently suppress Tg production. Inter-laboratory and inter-assay variability for Tg exceeds 25\% at low concentrations, so the same assay must be used for serial monitoring; patient-specific cut-points are more informative than absolute reference ranges. The 2015 ATA guidelines recommend that any detectable Tg on suppression with negative imaging should prompt further evaluation, while undetectable high-sensitivity Tg (<0.2 ng/mL) on suppression with negative TgAb and negative ultrasound predicts <1\% risk of recurrence.
...
\begin{itemize}
  ...
  \item \textbf{Undetectable Tg in congenital hypothyroidism:} Supports thyroid agenesis or athyreosis (no thyroid tissue present at all), which is the most common form of permanent congenital hypothyroidism in Caucasian populations.
...
\begin{itemize}
  \item \textbf{Post-total thyroidectomy with RAI ablation (on suppression):} <0.2 ng/mL (undetectable by high-sensitivity assay) --- this is the goal for excellent response
  ...
  \item \textbf{TgAb-negative status:} <20 IU/mL (lab-specific; Roche <115 IU/mL) --- Tg results are reliable only if TgAb is negative
  ...
  \item \textbf{Note:} Thyroglobulin reference intervals apply only to patients with intact thyroid glands. Post-thyroidectomy, any detectable Tg above assay functional sensitivity is considered abnormal (target is undetectable). Always use the same assay and laboratory for serial measurements because inter-assay variation at low concentrations can exceed 20\%.
\end{itemize}

\section*{Follow-up Tests}
\begin{itemize}
  \item \textbf{Anti-thyroglobulin antibodies (TgAb):} Must be measured on the same sample as every Tg measurement. Positive TgAb invalidates Tg as a tumor marker. Serial TgAb trends then become the surrogate DTC biomarker.
